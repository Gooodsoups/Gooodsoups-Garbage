...
vec4 bronze_color = vec4(184., 93., 14., 0.) / 255.;
...
bool line(vec2 uv, float offset, float width) {
    ...
    offset = offset + 0.35 * sin(bronze.x + TWO_PI * lines_offset);
    width = width + 0.005 * sin(bronze.x);
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    pixel = vec4(bronze_color.rgb * avg + tex.rgb * tex.a, pixel.a);
